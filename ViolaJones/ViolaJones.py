...
class ViolaJones:
    ...
    def train(self, x, y):
        """
        Using booseted cascade of features , each classifier is trained
        ...
        print(
            ...
            + str(int(pos_num))
            + " @ "
            + str(100.0 * pos_num / (pos_num + neg_num))
            + "%"
        )
        print(
            "  - Total non-faces: "
            + str(int(neg_num))
            + " @ "
            + str(100.0 * neg_num / (pos_num + neg_num))
            + "%"
        )
        print("  - Total samples: " + str(int(pos_num + neg_num)))
        print(f"  - Size (WxH): {img_w} x {img_h}")

        print("Generating integral images...")
        x_integ = np.array(list(map(lambda y: integeral_image(y), x)), dtype=np.uint32)
        ...
        if done_features is None:
            ...
            np.save(
                "applied_features" + self.featurespath + "xf" + ".npy", done_features
            )
        ...
        for i, t in enumerate(self.layers):
            print(
                "[CascadeClassifier] Training {} of out {} layers".format(
                    i + 1, len(self.layers)
                )
            )
    ...
    def classify(self, image, scale=1.0):
        ...
        """
        return self.classify_ii(integeral_image(image), scale)
