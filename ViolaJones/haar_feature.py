class HaarFeature:
    ...
    def get_haar_feature_value(self, integral_image, scale=1):
        ...
        white_positive_region_sum = sum(
            [
                rectangle.get_window_sum(integral_image, scale)
                for rectangle in self.positive_regions
            ]
        )

        # to get the black region value
        black_negative_region_sum = sum(
            [
                rectangle.get_window_sum(integral_image, scale)
                for rectangle in self.negative_regions
            ]
        )
