...
class RectangleWindow:
    ...
    def __init__(
        self,
        x_start_position_of_the_window,
        y_start_position_of_the_window,
        window_width,
        window_height,
    ):
    ...
    def get_window_sum(self, integral_image, scale=1.0):
        ...
        if x_window_start_top_left > 0:
            window_sum -= int(
                integral_image[x_window_start_top_left - 1, bottom_right_y]
            )

        if y_window_start_top_left > 0:
            window_sum -= int(
                integral_image[bottom_right_x, y_window_start_top_left - 1]
            )

        if x_window_start_top_left > 0 and y_window_start_top_left > 0:
            window_sum += int(
                integral_image[x_window_start_top_left - 1, y_window_start_top_left - 1]
            )
