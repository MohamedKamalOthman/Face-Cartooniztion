...
class AdaBoost:
    ...
    def train(self , features_values , labels , features):
        ...
        for t in range(self.T):
            ...
            clf, error, incorrectness = self.select_best(W_C, features_values, labels, weights)
            if error <= 0.5:
                # Compute alpha, beta
    ...
    def train_weak(self, features_values ,labels ,weights , features):
        ...
        for w,label in zip(weights , labels):
            if label == 1:
                ...
                total_neg += w
        bar = Bar('Processing Weak Classifiers', max=len(features_values), suffix='%(percent)d%% - %(elapsed_td)s - %(eta_td)s')
    ...
    def select_best(self,weak_clfs , features_values, labels , weights):
        ...
        for clf in weak_clfs:
            i += 1
            incorrectness = np.abs(clf.classify_with_feature(features_values[i]) - labels)
    ...
    def classify(self, integral_img, scale = 1.0):
        total = sum(list(map(lambda z: z[0] * z[1].classify(integral_img, scale), zip(self.alphas, self.clfs))))  # Weak classifiers
