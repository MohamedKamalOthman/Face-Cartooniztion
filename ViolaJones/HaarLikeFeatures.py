    ...
    for x  in range(row):
        for y in range(column):
            ...
            # previous row + previous column - (previous column and row) + the current point 
                ...
                integeral_img[x][y] = (integeral_img[x-1][y] + integeral_img[x][y-1] - integeral_img[x-1][y-1]) + image[x][y]
...
class HaarlikeFeatureDetector:
    ...
    # Understood
    @staticmethod
    def get_window_sum(integral_image, starting_row, starting_column, window_width, window_height):
        ...
        if( starting_row < 0 or starting_column < 0 or window_height < 0 or window_width < 0 ):
            return 0
        ...
        if( starting_column == 0 ):
            ...
            top_right = integral_image[ starting_row-1 + window_width , starting_column-1 ]
            
        if( starting_row == 0 ):
            bottom_left=0
        else:
            bottom_left= integral_image[ starting_row-1 , starting_column-1 + window_height ]
            
        bottom_right = integral_image[ starting_row-1 + window_width , starting_column-1 + window_height ]
    ...
    def determine_features(width, height):
        ...
        for haar_type in range( HaarlikeType.types_count.value ):
            ...
            haar_feature_window_size_x, haar_feature_window_size_y = __class__.haar_window[haar_type]
            ...
            for column in range(0, width - haar_feature_window_size_x + 1):
                for row in range(0, height - haar_feature_window_size_y + 1):
                    ...
                    features_count += int( (width-column) / haar_feature_window_size_x ) * int( (height-row) / haar_feature_window_size_y )
        ...
        for haar_type in range(HaarlikeType.types_count.value):
            ...
            # for each size
            for w in range(haar_feature_window_size_x, width+1, haar_feature_window_size_x):
                for h in range(haar_feature_window_size_y, height+1, haar_feature_window_size_y):
    ...
    def extract_features(integral_image, features_descriptions):
        ...
        for desc in features_descriptions:
            features[index]=  HaarlikeFeatureDetector.getSingleFeature( integral_image, 
                                                                        HaarlikeType(desc[0]),
                                                                        desc[1],
                                                                        desc[2],
                                                                        desc[3],
    ...
    def get_single_feature( integral_image, haar_type, x, y, w, h):
        ...
        if (haar_type== HaarlikeType.two_horizontal):
            white_region_sum =  HaarlikeFeatureDetector.get_window_sum ( integral_image , x      , y , w/2 , h)              # negative regions
            black_region_sum =  HaarlikeFeatureDetector.get_window_sum ( integral_image , x+ w/2 , y , w/2 , h)              # positive regions
        
        elif (haar_type== HaarlikeType.two_vertical):
            white_region_sum =  HaarlikeFeatureDetector.get_window_sum ( integral_image , x , y     , w , h/2)               # negative regions
            black_region_sum =  HaarlikeFeatureDetector.get_window_sum ( integral_image , x , y+h/2 , w , h/2)               # positive regions
        
        elif (haar_type== HaarlikeType.three_horizontal):
            white_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y, w/3, h) + HaarlikeFeatureDetector.get_window_sum (integral_image, x+ 2*w/3, y, w/3, h)                # negative regions
            black_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x+ w/3, y, w/3, h)                # positive regions
        
        elif (haar_type== HaarlikeType.three_vertical):
            white_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y, w, h/3) +  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y+2*h/3, w, h/3)                # negative regions
            black_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y + h/3, w, h/3)               # positive regions
        
        elif (haar_type== HaarlikeType.four_diagonal):
            white_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y, w/2, h/2)  +  HaarlikeFeatureDetector.get_window_sum (integral_image, x+w/2, y+h/2, w/2, h/2)               # negative regions
            black_region_sum =  HaarlikeFeatureDetector.get_window_sum (integral_image, x+w/2, y, w/2, h/2) +  HaarlikeFeatureDetector.get_window_sum (integral_image, x, y+h/2, w/2, h/2)                # positive regions
    ...
    def apply_features(features, training_data):
        ...
        for feature_description in features:
            feature_extractor = lambda intImg: HaarlikeFeatureDetector.getSingleFeature(intImg,
