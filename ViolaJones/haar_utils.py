...
def build_features(image_width, image_height, shift_amount=1, minimum_width=1, minimum_height=1):
        ...
        for window_height in range(minimum_height, image_height + 1):
            ...
            while x_top_left + window_width < image_width:
                ...
                while y_top_left + window_height < image_height:
                    ...
                    immediate = RectangleWindow( x_top_left , y_top_left , window_width , window_height )    # ==> |o|
                    
                    right     = RectangleWindow( x_top_left + window_width     , y_top_left , window_width , window_height )    # ==> | |o|
                    right_2 = RectangleWindow  ( x_top_left + window_width * 2 , y_top_left , window_width , window_height )    # ==> | | |o|
                    
                    bottom = RectangleWindow  ( x_top_left , y_top_left + window_height     , window_width , window_height )  # ==> | |/|o|
                    bottom_2 = RectangleWindow( x_top_left , y_top_left + window_height * 2 , window_width , window_height )  # ==> | |/| |/|o|
                    
                    bottom_right = RectangleWindow( x_top_left + window_width , y_top_left + window_height , window_width , window_height ) # ==> | |/| |o|
                    ...
                    if x_top_left + window_width * 2 < image_width and y_top_left + window_height * 2 < image_height:
...
def apply_features(features_integral_images, features):
    ...
    # X : features
    features_values = np.zeros((len(features), len(features_integral_images)), dtype=np.int32)
    ...
    for i, feature in bar.iter(enumerate(features)):
        ...
        features_values[i] = list(map(lambda integral_img: feature.get_haar_feature_value(integral_img), features_integral_images))
    bar.finish()
